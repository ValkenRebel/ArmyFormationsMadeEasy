...
namespace ArmyFormationsMadeEasy.Patches
{
    ...
    {
        static bool ErrorMsgDisplayed = false;

        static void Postfix(Agent agent)
        {
            try
            {
                if (agent == null || agent.State != AgentState.Active || agent.IsRetreating() || agent.Mission.CurrentState != Mission.State.Continuing || (!agent.Mission.IsFieldBattle && agent.Mission.MissionTeamAIType != Mission.MissionTeamAITypeEnum.FieldBattle))
                    return;
            ...
            catch (Exception ex)
            {
                ...
                {
                    MessageBox.Show($"Error: Army Formations Made Easy - UpdateFormationMovementPatch:\n\n{ex.ToStringFull()}");
